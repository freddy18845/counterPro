...
class SenderAccountSection extends StatefulWidget {
  final HomeFlowItem data;
  final Function(String accountNo) onTap;
  final VoidCallback onCancel;
  const SenderAccountSection({super.key, required this.onTap, required this.onCancel, required this.data});

  @override
  State<SenderAccountSection> createState() => _SenderAccountSectionState();
}

class _SenderAccountSectionState extends State<SenderAccountSection> {
  String _senderAccountNumber = '';
  @override
  void initState() {
    // TODO: implement initState
    setState(() {
      _senderAccountNumber = widget.data.senderAccount;
    });
    super.initState();
  }
  @override
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        NumberField(
          inputValue: _senderAccountNumber, hint: AppStrings.enterSenderAccountNumber,
        ),
         SizedBox(height: ConstantUtil.verticalSpacing),
        Expanded(
          child: NumPad(
            limit: 13,
            key: ValueKey('senderAccount-numpad'),
            initialValue: _senderAccountNumber,
            isAmount: false,
            onInput: (String value) {
              setState(() {
                _senderAccountNumber = value;
              });
            ...
            Expanded(
              child: Btn(
                ...
                onTap: () async {
                  ...
                  if (result == true) {
                    widget.onTap(_senderAccountNumber);
                  }
